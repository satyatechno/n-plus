...
export const useSpecialSectionSevenViewModel = () => {
  const navigation = useNavigation<NativeStackNavigationProp<RootStackParamList>>();

  const {
    data,
    loading,
    refetch: refetchSpecialSectionSeven
  ...
  useEffect(() => {
    ...
    setSectionHasData('specialSectionSeven', hasData);

    return () => {
      setSectionHasData('specialSectionSeven', false);
    };
  ...
  return {
    principal: data?.HomepageSpecialContent?.principal?.[0] ?? null,
