...
const useSpecialSectionFourViewModel = () => {
  ...
  const {
    ...
    loading,
    error,
    refetch: refetchSpecialSectionFour
  ...
  const onCardPress = (
    ...
    index?: number,
    itemType?: 'principal' | 'secondary' | 'carousel' | 'highlighted'
  ) => {
    if (!item?.slug) return;
    ...
    if (itemType === 'highlighted') {
      moleculeName = ANALYTICS_MOLECULES.HOME_PAGE.CARD_STYLE_3;
      displayIndex = (index ?? 0) + 1;
    } else if (itemType === 'principal' || index === 0) {
  ...
  useEffect(() => {
    ...
    setSectionHasData('specialSectionFour', hasData);

    return () => {
      setSectionHasData('specialSectionFour', false);
    };
...
export default useSpecialSectionFourViewModel;
